...
pub fn play_track(state: State<AppState>, path: String, position: Option<f32>) -> Result<(), String> {
    ...
    *player_state.current_source.lock().unwrap() = None;
    ...
    let source: Box<dyn Source<Item = f32> + Send> = if extension.eq_ignore_ascii_case("mp3") {
        ...
        match SymphoniaDecoder::new(&path) {
            Ok(mut decoder) => {
                ...
                match decoder.seek(duration) {
                    Ok(_) => {
                        let source: Box<dyn Source<Item = f32> + Send> = Box::new(crate::audio_decoder::SymphoniaSource::new(decoder));  
