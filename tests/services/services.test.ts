...
describe('eqService', () => {
  it('wraps EQ commands with correct payloads', async () => {
    ...
    await expectInvoke(() => setEqBandGain(3, -4.5), 'set_eq_band_gain', { band: 3, gain: -4.5 })
...
describe('mediaService', () => {
  it('wraps media commands with correct payloads', async () => {
    await expectInvoke(() => getTrackCoverPath('C:/music/a.mp3'), 'get_track_cover_path', {
      path: 'C:/music/a.mp3',
    })
    ...
    await expectInvoke(() => cleanCoverCache(512), 'clean_cover_cache_command', {
      maxCacheSizeMb: 512,
    })
    await expectInvoke(() => setCoverCachePath(null), 'set_cover_cache_path_command', {
      path: null,
    })
  })
